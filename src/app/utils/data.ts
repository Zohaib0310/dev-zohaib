import Dedication from "../assets/dedication.svg";
import DevOps from "../assets/devops.svg";
import FrontEnd from "../assets/FrontEnd.svg";
import Intelligence from "../assets/Intelligence.svg";
import Mobile from "../assets/Mobile.svg";
import Quotes from "../assets/quotes.svg";
import SmartWork from "../assets/smartWork.svg";
import Web from "../assets/WebApp.svg";
import MobileDevelopmentIcon from "../assets/mobile-development-icon.svg";
import WebDevelopmentIcon from "../assets/web-development-icon.svg";

export const ethicsItemData = [
  {
    id: 1,
    count: "01",
    title: "Dedication",
    img: Dedication,
    description:
      "A dedicated engineer, committed to excellence, pours heart and soul into every project, ensuring precision, quality, and timely delivery.",
  },
  {
    id: 2,
    count: "02",
    title: "Smart Work",
    img: SmartWork,
    description:
      "A smart-working engineer, utilizing efficiency and innovation, achieves exceptional outcomes through optimal processes and cutting-edge technology.",
  },
  {
    id: 3,
    count: "03",
    title: " Intelligence",
    img: Intelligence,
    description:
      "An intelligent engineer, combining knowledge and problem-solving skills, navigates complex challenges with strategic thinking and insightful solutions.",
  },
];

export const contributions = [
  {
    id: 1,
    count: 10,
    description: "Digital",
    secondDescription: "Products",
  },
  {
    id: 2,
    count: 12,
    description: "Open Source",
    secondDescription: "Project",
  },
  {
    id: 3,
    count: 10,
    description: "Million Lines of",
    secondDescription: "Codes",
  },
];

export const contactAddressData = [
  {
    id: 1,
    title: "Address",
    text: "Lahore, Punjab, Pakistan",
  },
  {
    id: 2,
    icon: "icon",
    title: "Email",
    text: "zohaib.anwar0310@gmail.com",
  },
  {
    id: 3,
    icon: "icon",
    title: "Phone",
    text: "+92 310 6212 262 ",
  },
];

export const reviews = [
  {
    id: 1,
    quoteImg: Quotes,
    description:
      "I have managed Zohaib, he is very supportive and strong knowledge in the latest technologies. He is one who never say no to anyone for getting technical problem, I strongly recommend him as a dynamic, creative and passionate smartest professional software engineer. Best of Luck!",
    userImg: "https://devman-react.vercel.app/img/testimonials/1.jpg",
    name: "Arslan Ahmad",
    role: "Full Stack Developer",
  },
  {
    id: 2,
    quoteImg: Quotes,
    description:
      "I've had the pleasure of working with Zohaib, a very humble person, eager to learn, autonomous and hard-worker. I recommend Abu Sufyan if you are looking for a skilled MERN Stack developer sensible to the context of your project and contributing to add value to your solution.",
    userImg: "https://devman-react.vercel.app/img/testimonials/1.jpg",
    name: "Anand",
    role: "Product Owner",
  },
  {
    id: 3,
    quoteImg: Quotes,
    description:
      "I have worked with Zohaib during our time developing Gigmi. He is an intelligent, an extremely quick learner and a superb problem solver. He digs deep and will have multiple solutions to a problem and will put all his energy to get things done and that too in a timely manner. Highly recommend!",
    userImg: "https://devman-react.vercel.app/img/testimonials/1.jpg",
    name: "Micheal",
    role: "Senior Functional Analyst",
  },
  {
    id: 4,
    quoteImg: Quotes,
    description:
      "I had the professional privilege of working with Zohaib. Technically skilled and always willing to help everyone on the team, he is someone I would always recommend. During the development of You & Me mobile application my team including Zohaib as well worked very passionately. Highly Recommended.",
    userImg: "https://devman-react.vercel.app/img/testimonials/1.jpg",
    name: "Angee",
    role: "Product Owner",
  },
];

export const portfolio = [
  {
    id: 1,
    title: "Carecloud EHR",
    subtitle: "ReactJS Developer",
    description:
      "Performance Analytics Systems that create dashboards for financial transparency, connected to your EHR, or our own",
    image: "https://devman-react.vercel.app/img/portfolio/2.jpg",
    tools:
      "ReactJS, NextJS, Typescript, WebServices, Elastic UI, Formik, Yup, React Query, Cypress",
    contentDirection: "float-right",
    background: "#7406CC",
    link: "https://www.climateclub.com/",
  },
  {
    id: 2,
    title: "Lago Design",
    subtitle: "ReactJS Developer",
    description:
      "Discover LAGO design for all areas of the home: timelessly elegant, made-in-Italy furniture to furnish with a unique and innovative style.",
    image: "https://devman-react.vercel.app/img/portfolio/3.jpg",
    tools:
      "ReactJS, ReactQuery, Material UI, JavaScript, Formik, StyledComponents, Redux Toolkit",
    contentDirection: "float-left",
    background: "#C291C4",
    link: "https://www.lagodesign.com/en/",
  },
  {
    id: 3,
    title: "Gigmi",
    subtitle: "ReactJS Developer",
    description:
      "GigMi is a platform that connects homeowners, contractors, and gig workers, facilitating efficient service exchanges and secure payments to enhance productivity and project management.",
    image: "https://devman-react.vercel.app/img/portfolio/1.jpg",
    tools: "Html, CSS, ReactJS, NodeJS, BootStrap",
    contentDirection: "float-left",
    background: "#2A2354",
    link: "https://gigmiapp.com/",
  },
  {
    id: 4,
    title: "Worship Serve",
    subtitle: "ReactJS Developer",
    description:
      "Worship Serve is an application designed to help churches manage volunteer schedules and streamline service planning, enhancing organizational efficiency and communication within religious communities..",
    image: "https://devman-react.vercel.app/img/portfolio/4.jpg",
    tools: "Html, CSS, Javascript, ReactJS, MUI",
    contentDirection: "float-right",
    background: "#D72587",
    link: "https://www.worshipserve.com/",
  },
];

export const blogsData = [
  {
    id: 1,
    img: "https://devman-react.vercel.app/img/news/1.jpg",
    title: "Content Writing",
    description: "How to Promote Your Blog in 2023",
    url: "https://www.cinextech.com/how-to-promote-your-blog-in-2023/",
  },
  {
    id: 2,
    img: "https://devman-react.vercel.app/img/news/1.jpg",
    title: "Content Writing",
    description: "How to Promote Your Blog in 2023",
    url: "https://www.cinextech.com/how-to-promote-your-blog-in-2023/",
  },
  {
    id: 3,
    img: "https://devman-react.vercel.app/img/news/1.jpg",
    title: "Content Writing",
    description: "How to Promote Your Blog in 2023",
    url: "https://www.cinextech.com/how-to-promote-your-blog-in-2023/",
  },
];

export const developmentData = [
  {
    id: 1,
    img: Mobile,
    title: "Mobile App Development",
    description:
      " React Native, Authentication, Google Maps, React Native Gifted Chats",
    bg: "#6E7FD1",
    radius: "rounded-tl-xl",
  },
  {
    id: 2,
    img: Web,
    title: "Frontend Development",
    description:
      "Html, CSS, Javascript, TypeScript, ReactJS, NextJS, Redux Toolkit, Redux Saga, React Query, MUI, TailwindCSS, Elastic UI, Styled Components ",
    bg: "#343C55",
    radius: "rounded-tr-xl",
  },
  {
    id: 3,
    img: FrontEnd,
    title: "Backend Development",
    description: " NodeJS, ExpressJS, MongoDB  ",
    bg: "#142FB5",
    radius: "rounded-bl-xl",
  },
];

export const SkillsItemsData = [
  {
    id: 1,
    percentage: 85,
    label: "ReactJS",
  },
  {
    id: 2,
    percentage: 75,
    label: "NextJS",
  },
  {
    id: 3,
    percentage: 60,
    label: "NodeJS",
  },

  {
    id: 4,
    percentage: 65,
    label: "Express js",
  },

  {
    id: 5,
    percentage: 85,
    label: "TypeScript",
  },
];

export const HOW_CAN_I_HELP_YOU = [
  {
    title: "App Development",
    description:
      "I can build scalable and secure mobile applications for both Android and iOS platforms using React Native.",
    percentage: "70%",
    icon: MobileDevelopmentIcon,
    width: "w-8/12",
  },
  {
    title: "Front-end Development",
    description:
      "I can develop responsive web-applications using modern front-end technologies like React js, Next js, and Redux.",
    percentage: "83%",
    icon: WebDevelopmentIcon,
    width: "w-11/12",
  },
  {
    title: "Backend Development",
    description:
      "I can develop robust and scalable back-end systems using Node js, Nest js, Express js, PostgreSQL and MongoDB.",
    percentage: "75%",
    icon: WebDevelopmentIcon,
    width: " w-5/6",
  },
];
